...
### Krok 48 — import CSV listy pozycji (2026-08-30)

`POST /portfolios/{portfolio_id}/holdings/import`, format kanoniczny z planu
`symbol;ilość;cena_nabycia`. **Bez migracji Alembic** — patrz decyzja 1 niżej.

**Dwie decyzje użytkownika, obie zmieniają plan etapu 9 (decyzja 2 z 2026-08-10):**

1. **`cena_nabycia` idzie do istniejącej kolumny `holdings.avg_cost`, nie do nowej
   `acquisition_price_note`.** Powód znaleziony przy wejściu w kod: `holdings` **już ma**
   `avg_cost` + `cost_currency` (z CHECK-iem `avg_cost_needs_currency`), a `service.value_position`
   liczy z nich `unrealized_pl`. Dodanie drugiej kolumny na cenę nabycia dałoby dwa równoległe
   pola o tym samym znaczeniu, z których jedno celowo nic nie robi — plus migrację i dokładnie
   to ryzyko pełzania ku transakcjom, które plan każe kontrolować po tym kroku. Oryginalny
   argument za osobną kolumną („liczba bez waluty niczego nie policzy") znika, bo walutę bierzemy
   z `assets.currency`: cena nabycia instrumentu jest w walucie jego notowania.
   **Konsekwencja:** `acquisition_price_note` nie powstanie; `/kontrola-zakresu` po tym kroku
   dotyczy teraz importu, a nie nowej kolumny.

2. **Import SUMUJE ilości z istniejącą pozycją**, nie nadpisuje ich i nie zgłasza konfliktu.
   Zaproponowałem wariant „zgłoś konflikt, nie ruszaj pozycji"; użytkownik wybrał sumowanie.
   Konsekwencja, którą trzeba było czymś domknąć: po zsumowaniu ilości stara `avg_cost` opisuje
   ilość, której w portfelu już nie ma. Rozwiązanie: **średnia ważona ilością**
   `(q_old × c_old + q_new × c_new) / (q_old + q_new)` w `service.merge_quantity_and_cost`
   (funkcja czysta, testy na ręcznie policzonych liczbach). To **nie jest** FIFO ani zrealizowany
   P/L (CLAUDE.md #12, Etap 21) — liczymy średnią z cen zadeklarowanych przez użytkownika, bez
   przepływów pieniężnych, dat nabycia i rozchodu. Gdy któraś strona nie zna ceny albo waluty się
   różnią, `avg_cost` idzie na `NULL` i powód wraca w raporcie (zasada #15: utrata danych nie
   może być cicha).

**Co powstało:**
- `backend/app/modules/portfolio/csv_import.py` — parser czysty, bez I/O. Przyjmuje BOM, nagłówek
  opcjonalny, przecinek dziesiętny i spacje jako separator tysięcy (także U+00A0/U+202F z Excela).
  Odrzuca liczbę z przecinkiem I kropką naraz (`1,234.56` vs `1.234,56` dają inny wynik i nie ma
  jak rozstrzygnąć), `NaN`/`Infinity` oraz powtórzony symbol w jednym pliku.
- `service.import_holdings_csv` + `repository.apply_import` (**jeden `commit()` na plik**,
  bump `holdings_version` raz — wersja cache oznacza „skład się zmienił", nie „ile pozycji")
  oraz `repository.get_assets_by_symbols` (jedno zapytanie na plik, lookup bez rozróżniania
  wielkości liter, pomija `is_active = false`).
- `dry_run` w kontrakcie: ten sam raport, zero zapisu. Frontend woła najpierw z `true` —
  sumowania ilości nie da się cofnąć jednym przyciskiem.
- Frontend: `lib/holdingsImport.ts`, `components/forms/HoldingsImport.tsx` (podgląd → zapis),
  wejście z dashboardu obok „Dodaj pozycję" i ze stanu pustego. Status wiersza niesie SŁOWO
  („dodano"/„scalono"/„pominięto"), kolor jest tylko wzmocnieniem (§21).
- Testy: 21 jednostkowych (`tests/unit/test_csv_import.py`), 10 integracyjnych
  (`tests/integration/test_holdings_import.py`, w tym 409 przy kolizji
...
**Złapane własnym testem, warte zapamiętania:** pierwsza wersja heurystyki nagłówka („kolumna
ilości nie jest liczbą") zjadała jednowierszowy plik z literówką w ilości — import kończył się
raportem „0 pozycji, 0 błędów", czyli milczał dokładnie tam, gdzie ma powiedzieć, co poprawić.
Warunek zaostrzony: nagłówek to wiersz, w którym **żadne** pole nie jest liczbą.

**Weryfikacja:** backend **589 passed** (`pytest -m "not network"`, 5:45), ruff format + ruff
check + mypy czyste, frontend: lint, `tsc --noEmit`, Vitest **95 passed**, `next build` zielony.

**Notatka operacyjna (druga wpadka tego samego rodzaju):** pierwszy przebieg suity dał 15 błędów
w `test_analytics.py`, bo równolegle uruchomiłem testy pojedynczego pliku na tej samej bazie
deweloperskiej. Po przerwaniu i jednym czystym przebiegu — zero błędów. **Jedna sesja pytest
naraz**, zawsze; przy dwóch werdykt suity jest bezwartościowy, a błędy lądują w plikach, które
nie mają nic wspólnego ze zmianą.

**Zostaje po kroku 48 (nieblokujące):**
- Brak eksportu do CSV — import jest jednokierunkowy.
- `dry_run` liczy raport dwa razy (podgląd + zapis), więc między jednym a drugim skład portfela
  może się zmienić w innej karcie; raport z podglądu nie jest rezerwacją.
- Nie ma testu Playwright na sam upload pliku — pokrycie kończy się na warstwie HTTP i na
  jednostkowych testach parsera.
- `test_holdings_import.py` nie zakłada kursu `XTS`, więc nie sprawdza wyceny importowanych
  pozycji (robi to `test_holdings.py`); wspólny wiersz `fx_rates` sprzęgałby oba pliki przez
  globalny stan — jeden nieudany teardown wywalał setup każdego następnego przebiegu.
...
| 2026-09-06 | **Etap AI-1a (moduł AI Fund, ADR-104): modele i migracja.** Sześć tabel w `backend/app/modules/ai_fund/models.py` (`AIFundSession`, `AIAgentLog`, `AssetVibeMetric`, `AssetAnalystRating`, `AIPrediction`, `AILesson` z kolumną `embedding Vector(768)` pod przyszły RAG) + migracja `20260906_ai_fund_models.py` z RLS dla trzech tabel własnościowych (wzorem `holdings`). Obraz Postgresa zmieniony na `pgvector/pgvector:pg16` w `docker-compose.yml`, `docker-compose.prod.yml` i `ci.yml` — zwykły `postgres:16` nie ma rozszerzenia `vector`. Code-review (`code-reviewer`) złapał dwa blokujące: zły `down_revision` migracji (wskazywał na niezacommitowaną rewizję modułu `push` z innej, równoległej sesji) i właśnie brak `pgvector` w obrazach — oba naprawione. Dodane testy RLS (`test_ai_fund_session_dziedziczy_wlasciciela_po_portfelu`, `test_ai_agent_log_dziedziczy_wlasciciela_przez_sesje`, dwupoziomowy łańcuch portfolio→session→logs). | Commit `a3b91ed`, poprzedzony `docs(adr-104)` (`7fb4d3d`) opisującym decyzję o rozszerzeniu zakresu. Zweryfikowane: ruff+mypy czyste, cykl `alembic upgrade→downgrade→upgrade`, **598 passed, 3 deselected**. Zakres celowo ograniczony do modeli+migracji — bez silnika agentów, endpointów i joba `ingest_analyst_ratings` (AI-1b, następny krok). Working tree ma równolegle niezacommitowany, niepowiązany moduł `push` (krok 50, inna sesja) — świadomie pominięty przy stagingu i commicie tego etapu. |
